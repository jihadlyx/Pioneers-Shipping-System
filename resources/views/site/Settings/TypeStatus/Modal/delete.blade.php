<div id="DeleteState{{ $state->id_state }}" x-transition="" class="modal fixed left-0 top-0 z-999999 hidden h-full min-h-screen w-full items-center justify-center bg-black/90 px-4 py-5">
    <div  class="w-full max-w-142.5 rounded-lg bg-white px-8 py-12 text-center dark:bg-boxdark md:px-17.5 md:py-15">
      ...
      <div class="-mx-3 flex flex-wrap gap-y-4">
        ...
        <div class="w-full px-3 2xsm:w-1/2">
          <form action="{{ route('status.destroy', ['page_id' => $id_page, 'id_state' => $state->id_state]) }}" method="POST">
           @csrf
              @method('DELETE')
