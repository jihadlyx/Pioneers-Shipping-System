    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex text-lg font-medium flex-wrap items-center gap-3">
                ...
                <li>
                    <a class="flex items-center gap-3 font-medium" href="/roles/7">
                ...
                @if($isCreate || $isShowTrash)
                    <div class="flex items-center gap-1 border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                        @if($isCreate)
                            ...
                            @include('site.settings.Roles.modal.add')
                        @endif
                        @if($isShowTrash)
                            <a href="{{ route('roles.getTrash', ["page_id" => 10]) }}"
                                ...
                                <tr data-index="{{ $index }}">
                                    <td class="px-4 py-5">
                                        <div class="text-gray-100 flex items-center gap-1">
                                            @if($isUpdate)
                                                <button data-target="EditRole{{ $role->role_id }}" title="تعديل" class="hover:text-primary">
                                            ...
                                            @if($isDelete && $role->isHasMany() )
                                                    <button data-target="DeleteRole{{ $role->role_id }}" title="حذف" class="hover:text-meta-1 button-icon">
                                    ...
                                    <td>
                                        <button type="submit"
                                            ...
                                            <a href="{{ route('materialRoles.show', ['page_id' => $page_id, 'role_id' => $role->role_id]) }}">
                                                أذونات
                                            </a>
                                        </button>
                                    </td>
                                    @include('site.settings.Roles.modal.delete')
                                    @include('site..settings.Roles.modal.edit')
                                </tr>
                            @endforeach
