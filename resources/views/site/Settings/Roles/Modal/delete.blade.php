<div id="DeleteRole{{ $role->role_id }}" x-transition="" class="modal fixed left-0 top-0 z-999999 hidden h-full min-h-screen w-full items-center justify-center bg-black/90 px-4 py-5">
    <div  class="w-full max-w-142.5 rounded-lg bg-white px-8 py-12 text-center dark:bg-boxdark md:px-17.5 md:py-15">
      ...
      <p class="mb-10 font-medium">
        اذا قمت بحذف هذه الصلاحية فسيتم نقله الى سلة المحذوفات
      </p>
      <div class="-mx-3 flex flex-wrap gap-y-4">
        ...
        <div class="w-full px-3 2xsm:w-1/2">
          <form action="{{ route("roles.destroy", ['page_id' => 7, 'role_id' => $role->role_id]) }}" method="POST">
              @csrf
              @method("DELETE")
