<div id="EditRole{{ $role->id_role }}" x-transition=""
    ...
    <div
        ...
        <form action="{{ route('roles.update', ['id_role' => $role->id_role, 'page_id' => 7]) }}" method="POST" class="needs-validation" novalidate>
            @csrf
            @method("PATCH")
            <div class="p-6.5">
                <div class="mb-4.5 flex flex-col gap-6 xl:flex-row">
                    ...
                    <div class="w-full xl:w-1/2">
                        ...
                        <input type="text" name="title" value="{{ $role->title }}" placeholder="ادخل اسم الصلاحية"
                            class="w-full rounded border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                            required />
                        <div class="invalid-feedback pr-4 text-red-500 mt-1 text-sm">
                            الرجاء ادخل حقل اسم الصلاحية
                        </div>
                    </div>
                </div>
