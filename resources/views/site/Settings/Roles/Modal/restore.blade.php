<div id="RestoreRole{{ $role->id_role }}" x-transition=""
    ...
    <div class="w-full max-w-142.5 rounded-lg bg-white px-8 py-12 text-center dark:bg-boxdark md:px-17.5 md:py-15">
        ...
        <h3 class="mt-5.5 pb-2 text-xl font-bold text-black dark:text-white sm:text-2xl">
            هل انت متأكد من استعادة هذه الصلاحية
        </h3>
        <div class="-mx-3 flex flex-wrap gap-y-4">
            ...
            <div class="w-full px-3 2xsm:w-1/2">
                <form action="{{ route('roles.restore', ['page_id' => $id_page, 'id_role' => $role->id_role]) }}" method="POST">
                    @csrf
                    @method("PATCH")
                    <button type="submit"
                            class="block w-full rounded border border-meta-5 bg-meta-5 p-3 text-center font-medium text-white transition hover:bg-oparole-90">
                        نعم
                    </button>
                </form>
            </div>
