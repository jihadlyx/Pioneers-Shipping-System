<div id="DeleteBranch{{ $branch->branch_id }}" x-transition=""
    ...
    <div class="w-full max-w-142.5 rounded-lg bg-white px-8 py-12 text-center dark:bg-boxdark md:px-17.5 md:py-15">
        ...
        <h3 class="mt-5.5 pb-2 text-xl font-bold text-black dark:text-white sm:text-2xl">
            هل انت متأكد من حذف هذه الفرع
        </h3>
        <p class="mb-10 font-medium">
            اذا قمت بحذف هذ الفرع فسيتم نقله الى سلة المحذوفات
        </p>
        <div class="-mx-3 flex flex-wrap gap-y-4">
            ...
            <div class="w-full px-3 2xsm:w-1/2">
                <form action="{{ route('branches.destroy', ['page_id' => $id_page, 'branch_id' => $branch->branch_id]) }}" method="POST">
                    @csrf
                    @method("DELETE")
                    <button type="submit"
